...
import com.firecode.dischole.URLCanonicalizer;
import com.firecode.dischole.WebURL;
import com.firecode.dischole.authentication.AuthInfo;
import com.firecode.dischole.authentication.BasicAuthInfo;
import com.firecode.dischole.authentication.FormAuthInfo;
import com.firecode.dischole.authentication.NtAuthInfo;
...
public class PageFetcher {
    protected static final Log logger = LogFactory.getLog(PageFetcher.class);
    ...
    public PageFetcher(CrawlConfig config) {
        ...
        if (config.getProxyHost() != null) {
            ...
            logger.debug(String.format("Working through Proxy: {}", proxy.getHostName()));
        }
    ...
    private void addBasicCredentials(BasicAuthInfo authInfo,
        ...
        logger.info(String.format("BASIC authentication for: {}", authInfo.getLoginTarget()));
    ...
    private void addNtCredentials(NtAuthInfo authInfo, Map<AuthScope, Credentials> credentialsMap) {
        logger.info(String.format("NT authentication for: {}", authInfo.getLoginTarget()));
        try {
    ...
    private void doFormLogin(FormAuthInfo authInfo) {
        logger.info(String.format("FORM authentication for: {}", authInfo.getLoginTarget()));
        String fullUri =
        ...
        try {
            ...
            logger.debug(String.format("Successfully request to login in with user: {} to: {}", authInfo.getUsername(),
                    authInfo.getHost()));
        } catch (ClientProtocolException e) {
            logger.error(String.format("While trying to login to: {} - Client protocol not supported",
                    authInfo.getHost(), e));
        } catch (IOException e) {
            logger.error(String.format("While trying to login to: {} - Error making request", authInfo.getHost(),
                    e));
        }
    }

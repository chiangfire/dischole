...
public class TLDList {
    ...
    private static final Log logger = LogFactory.getLog(TLDList.class);
    ...
    private static int readStream(InputStream stream, Set<String> tldSet) {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(stream))) {
            ...
            logger.warn(String.join(": ","Error while reading TLD-list", e.getMessage()));
        }
        return tldSet.size();
    }
    ...
    private static Set<String> tldSupplier() {
        ...
        if (onlineUpdate) {
            try (InputStream in = new URL(url).openStream()) {
                ...
                logger.info(String.format("Obtained {} TLD from URL {}", n, url));
            } catch (Exception e) {
                logger.error(String.format("Couldn't fetch the online list of TLDs from: {}",url, e));
    ...
    private static void loadFromFiles(Set<String> tldSet) {
        try (InputStream tldFile = FileUtils.openInputStream(new File(TLD_NAMES_TXT_FILENAME))) {
            logger.debug(String.format("Fetching the list from a local file {}", TLD_NAMES_TXT_FILENAME));
            int n = readStream(tldFile, tldSet);
            logger.info(String.format("Obtained {} TLD from local file {}", n, TLD_NAMES_TXT_FILENAME));
        } catch (FileNotFoundException e) {
            logger.info(String.format("File not found: {}", TLD_NAMES_TXT_FILENAME));
        } catch (IOException e) {
            logger.error(String.format("Couldn't read the TLD list from file {}", TLD_NAMES_TXT_FILENAME));
        }

        try (InputStream tldFile = TLDList.class.getClassLoader()
                .getResourceAsStream(TLD_NAMES_TXT_FILENAME)) {
            int n = readStream(tldFile, tldSet);
            logger.info(String.format("Obtained {} TLD from packaged file {}", n, TLD_NAMES_TXT_FILENAME));
        } catch (IOException e) {

...
public class CrawlController {
    ...
    static final Log logger = LogFactory.getLog(CrawlController.class);
    ...
    protected <T extends WebCrawler> void start(final WebCrawlerFactory<T> crawlerFactory,
        ...
        try {
            ...
            for (int i = 1; i <= numberOfCrawlers; i++) {
                ...
                threads.add(thread);
                logger.info(String.format("Crawler {} started", i));
            }
                        ...
                        synchronized (waitingLock) {
                            ...
                            while (true) {
                                ...
                                for (int i = 0; i < threads.size(); i++) {
                                    ...
                                    if (!thread.isAlive()) {
                                        if (!shuttingDown) {
                                            logger.info(String.format("Thread {} was dead, I'll recreate it", i));
                                            T crawler = crawlerFactory.newInstance();
    ...
    public void addSeed(String pageUrl, int docId) {
        ...
        if (canonicalUrl == null) {
            logger.error(String.format("Invalid seed URL: {}", pageUrl));
        } else {
            if (docId < 0) {
                ...
                try {
                    ...
                    logger.error(String.format("Could not add seed: {}", e.getMessage()));
                }
            }
            ...
            if (robotstxtServer.allows(webUrl)) {
                ...
                logger.warn(String.format("Robots.txt does not allow this seed: {}", pageUrl));
            }
        }
    }
    ...
    public void addSeenUrl(String url, int docId) {
        ...
        if (canonicalUrl == null) {
            logger.error(String.format("Invalid Url: {} (can't cannonicalize it!)", url));
        } else {
            try {
                docIdServer.addUrlAndDocId(canonicalUrl, docId);
            } catch (Exception e) {
                logger.error(String.format("Could not add seen url: {}", e.getMessage()));
            }
        }
    }
